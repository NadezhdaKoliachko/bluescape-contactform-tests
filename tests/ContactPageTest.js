...
requiredFields.add([contactPage.contactFormFields.name.label, contactPage.contactFormFields.name.input]);
requiredFields.add([contactPage.contactFormFields.email.label,  contactPage.contactFormFields.email.input]);
...
Data(requiredFields).Scenario('Test Contact Form required field',  async ({current}) => {
    assert.equal(await contactPage.checkContactFormFieldIsRequired(current.fieldLabel, current.inputLocator),
        true);
});

Scenario('Test Website input field is required to be URL', async ({I}) => {
    await I.seeAttributesOnElements(contactPage.contactFormFields.website.input, {type: "url"});
});
